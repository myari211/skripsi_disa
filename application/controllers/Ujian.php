...
class Ujian extends CI_Controller {
	...
	public function __construct(){
		...
		$this->load->model('Freq_missed_lo_model', 'freq');
	...
	public function list()
	{
		...
		$data = [
			...
			'judul'		=> 'Ujian',
			'subjudul'	=> 'List Ujian',
			'siswa' 		=> $this->ujian->getIdSiswa($user->username),
		];
		$this->load->view('_templates/dashboard/_header.php', $data);
    ...
    public function simpan_akhir()
	{
		...
		foreach ($pc_jawaban as $jwb) {
			...
			if ($jawaban == $cek_jwb->jawaban) {
				$jumlah_benar++;
				array_push($correct_answer, $id_soal);
			} else {
				$jumlah_salah++;	
				array_push($hambatan_siswa, $id_soal);
			}
		}
		...
		$d_update = [
			'hambatan_siswa' => implode(",",$hambatan_siswa), // hasilnya "1,5,6,8,11,65"
